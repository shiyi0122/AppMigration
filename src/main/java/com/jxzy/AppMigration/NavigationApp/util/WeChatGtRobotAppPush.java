...
public class WeChatGtRobotAppPush {
    ...
    //招商小程序推送
    @SuppressWarnings("all")
    public static String singlePushBusinessApp(String ClientId, String Message, String msg) throws Exception {
        System.out.println("控制端个推加密后数据:" + Message);
        IGtPush push = new IGtPush("http://sdk.open.api.igexin.com/apiex.htm", "bNQOqCa3zU8QOihB8KDAj4", "hVYPRKe1sz7E8oM4p9tMJ");
        System.out.println("个推id:" + ClientId + "消息:" + Message + "内容为:" + msg);

        TransmissionTemplate template = new TransmissionTemplate();
        template.setAppId("u9kv9IlHVN80mlWeSsWfV5");
        template.setAppkey("bNQOqCa3zU8QOihB8KDAj4");
        // 透传消息设置，1为强制启动应用，客户端接收到消息后就会立即启动应用；2为等待应用启动
        template.setTransmissionType(2);
        template.setTransmissionContent(Message);
        ...
        message.setPushNetWorkType(0);
        Target target = new Target();
        target.setAppId("u9kv9IlHVN80mlWeSsWfV5");
        target.setClientId(ClientId);
        IPushResult ret = null;
        try {
            ret = push.pushMessageToSingle(message, target);
    ...
    //游娱go推送
    @SuppressWarnings("all")
    public static String singlePushYYGApp(String ClientId, String Message, String msg) throws Exception {
...
//        IGtPush push = new IGtPush("http://api.getui.com/apiex.htm", "qjDrXtWhsFAIde8DnQy0X5", "KUyikm8QdT6gQP1HA1s7d1");
        //新版本推送
        IGtPush push = new IGtPush("http://api.getui.com/apiex.htm", "bNQOqCa3zU8QOihB8KDAj4", "Fsq0FnYWPl8tyjDbxNu5q6");
